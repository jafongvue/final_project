    ...
    <v-hover v-slot="{ hover }" open-delay="200">
      ...
      >
        ...
        <v-card-actions class="white justify-center">
          <v-col>
            <div class="d-flex">
              <div><h4>{{ pro.proName }}</h4>
              <h4 id="qty">ຈຳນວນ: {{ pro.qty }}</h4></div>
              <v-spacer></v-spacer>
          <h4 id="price">{{ currency(pro.price) }} ₭</h4>
            </div>
          
          <p id="detail" class="text-subtitle-2" v-if="pro.details"><i>{{ pro.details }}</i></p>
...
export default {
  ...
  methods: {
    ViewDetail(pro) {
      // alert(pro)
      this.$router.push(`/view.customer/productDetail/${pro.pro_id}`);
    },
    AddToCart(
      pro_id,
      proName,
      ...
      if (
        this.$cookies.get("cus_id") == undefined ||
        this.$cookies.get("cus_id") == null ||
        this.$cookies.get("cus_id") == ""
      ) {
        this.$router.push("/views.BillionFashion/customer_login");
        return;
      }
      this.$store.dispatch("orderBuy_cust/AddProductToCart", {
        pro_id,
        proName,
        typeName,
        unitName,
